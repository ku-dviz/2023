---
title: 'Exercises: Visualising Networks'
output:
...
using plot, setting node attributes and plotting some features of the network.

First, read in the network data - the edgelist is in the form of a .txt file, so
...
After reading the data, drop the repeated edges from the edgelist using distinct
in dplyr. Then make sure the ordering of the nodes from the graph vertices matches 
the edgelist using match. 
...
(hint: use as.factor to convert the roles to factors, and then assign the colors as node
attributes using V(graph)$color). 
...
distributions look significantly for the different roles? 
